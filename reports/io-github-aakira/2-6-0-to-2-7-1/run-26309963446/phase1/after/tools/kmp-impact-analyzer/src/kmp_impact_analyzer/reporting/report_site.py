...
def _render_propagation_graph_svg(consolidated: ConsolidatedResult) -> str:
    ...
    #
    # IMPORTANT: the only edges we know with certainty are DEPENDENCY -> each
    # direct importer (those files literally import the bumped library).
    # The further hops (direct -> transitive dist 1 -> transitive dist 2+)
    # come from a BFS over the file dependency graph; we do NOT preserve
    # the per-file adjacency in the FileImpact contract, so drawing N x M
    # lines between columns would falsely suggest every direct file
    # connects to every transitive file. To avoid that "todo con todo"
    # ambiguity, we render a single guide line per column-pair (centred
    # mid-column) and draw it dashed + low-opacity so the reader sees the
    # propagation direction without misreading it as a real adjacency.
    dep_cx = col_x[0] + col_w
    dep_cy = dep_box_y + (box_h + 10) // 2
    for _, bx, by in direct_boxes:
        parts.append(
            f"<line x1='{dep_cx}' y1='{dep_cy}' x2='{bx}' y2='{by + box_h // 2}' "
            f"stroke='#4338ca' stroke-width='1.5' opacity='0.45'/>"
        )

    def _column_centre_y(boxes: list[tuple[str, int, int]]) -> int | None:
        if not boxes:
            return None
        ys = [by + box_h // 2 for _, _, by in boxes]
        return (min(ys) + max(ys)) // 2

    direct_cy = _column_centre_y(direct_boxes)
    trans1_cy = _column_centre_y(trans1_boxes)
    trans2_cy = _column_centre_y(trans2_boxes)

    if direct_cy is not None and trans1_cy is not None:
        parts.append(
            f"<line x1='{col_x[1] + col_w}' y1='{direct_cy}' "
            f"x2='{col_x[2]}' y2='{trans1_cy}' "
            f"stroke='#f59e0b' stroke-width='2' stroke-dasharray='6 5' opacity='0.6'/>"
        )
        parts.append(
            f"<text x='{(col_x[1] + col_w + col_x[2]) // 2}' "
            f"y='{(direct_cy + trans1_cy) // 2 - 6}' text-anchor='middle' "
            f"fill='#b45309' font-size='9' font-weight='600'>imports of imports</text>"
        )

    if trans1_cy is not None and trans2_cy is not None:
        parts.append(
            f"<line x1='{col_x[2] + col_w}' y1='{trans1_cy}' "
            f"x2='{col_x[3]}' y2='{trans2_cy}' "
            f"stroke='#f97316' stroke-width='2' stroke-dasharray='6 5' opacity='0.6'/>"
        )
        parts.append(
            f"<text x='{(col_x[2] + col_w + col_x[3]) // 2}' "
            f"y='{(trans1_cy + trans2_cy) // 2 - 6}' text-anchor='middle' "
            f"fill='#9a3412' font-size='9' font-weight='600'>further hops</text>"
        )
...
<!-- Tools used (KSP / Detekt / Kover only — see _render_tool_badges) -->
    ...
    <details style="margin:14px 0 0 0;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 14px;">
      ...
      <div style="margin-top:10px;font-size:12.5px;line-height:1.7;color:#334155;">
        <ul style="margin:0 0 0 1.2em;padding:0;">
          ...
          <li><strong>Solid violet lines</strong> are real edges: each direct importer is wired to the dependency.</li>
          <li><strong>Dashed amber/orange lines</strong> are propagation arrows between columns &mdash; they show direction (red &rarr; amber &rarr; orange), not a 1-to-1 adjacency between files. The exact per-file edges live in the BFS graph and are listed in the traceability table below.</li>
        </ul>
      </div>
    </details>
    <div class="caption">
      Each box is a <code>.kt</code> file. Colour = relation to the bumped dependency.
      Solid lines = direct import edges. Dashed lines = propagation direction across columns.
    </div>
  </div>
</div>
